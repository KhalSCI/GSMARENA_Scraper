...
class CustomRedirectMiddleware(RedirectMiddleware):
    ...
    def process_response(self, request, response, spider):
        ...
        if 'Location' in response.headers:
            location = response.headers['Location'].decode(response.encoding)
            spider.logger.info(f'HTTP redirect to {location}')
            # Modify the location to point to the desktop version
            if 'm.gsmarena.com' in location:
                location = location.replace('m.gsmarena.com', 'www.gsmarena.com')
                spider.logger.info(f'Modified redirect to {location}')
                return self._redirect(request, location, spider)

        return response

    def _redirect(self, request, location, spider):
        redirected_url = request.urljoin(location)
